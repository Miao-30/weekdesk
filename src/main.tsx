...
const initial:AppState={schedule:null,todos:[],ui:{activeView:'schedule',focusTodoId:null},preferences:{topmost:false,opacity:.94,fontSize:14,todoFontSize:14,autoStart:false,showInactive:true,showWeekend:true,rememberLogin:true,theme:'soft',todoSort:'due',todoFilter:'today'},sync:{status:'idle',message:'登录教务系统，开启你的周课表'}};
...
function App(){
 ...
 return <main data-theme={p.theme??'soft'} style={{'--surface-alpha':p.opacity,'--base-font':p.fontSize+'px','--font-scale':p.fontSize/14,'--todo-font':todoFont+'px','--todo-scale':todoFont/14,'--schedule-dialog-scale':Math.min(1,p.fontSize/14),'--todo-dialog-scale':Math.min(1,todoFont/14)} as React.CSSProperties}>
  ...
  {settings&&<div className="modal-backdrop" onClick={()=>setSettings(false)}>
    <section className={`modal settings ${state.ui.activeView}`} role="dialog" aria-modal="true" aria-label="程序设置" onClick={e=>e.stopPropagation()}>
      <button className="modal-close" onClick={()=>setSettings(false)}>×</button>
      <div className="eyebrow">让周序适合你的桌面</div><h2>程序设置</h2>
      <fieldset className="theme-picker"><legend>界面主题</legend>
        <button className={(p.theme??'soft')==='soft'?'active':''} onClick={()=>save({theme:'soft'})}><span className="theme-preview soft"><i/><i/><i/><i/></span><b>柔和浅灰</b><small>低饱和、安静耐看</small></button>
        <button className={p.theme==='bright'?'active':''} onClick={()=>save({theme:'bright'})}><span className="theme-preview bright"><i/><i/><i/><i/></span><b>明亮纯色</b><small>清爽背景、鲜亮课程色</small></button>
      </fieldset>
      <label>背景不透明度 <b>{Math.round(p.opacity*100)}%</b><input type="range" min="72" max="100" value={p.opacity*100} onChange={e=>save({opacity:+e.target.value/100})}/></label>
      <label>课表大小 <b>{p.fontSize}px</b><input type="range" min="6" max="18" value={p.fontSize} onChange={e=>save({fontSize:+e.target.value})}/></label>
      <label>待办大小 <b>{todoFont}px</b><input type="range" min="6" max="18" value={todoFont} onChange={e=>save({todoFontSize:+e.target.value})}/></label>
      <label className="check"><span>显示周六和周日</span><input type="checkbox" checked={p.showWeekend} onChange={e=>save({showWeekend:e.target.checked})}/></label>
      <label className="check"><span>记住教务登录状态</span><input type="checkbox" checked={p.rememberLogin??true} onChange={e=>save({rememberLogin:e.target.checked})}/></label>
      <label className="check"><span>始终显示在其他窗口上方</span><input type="checkbox" checked={p.topmost} onChange={e=>save({topmost:e.target.checked})}/></label>
      <label className="check"><span>开机启动</span><input type="checkbox" checked={p.autoStart} onChange={e=>save({autoStart:e.target.checked})}/></label>
      <p>开启“记住教务登录状态”后，登录会话只保存在这台电脑；学校服务器使会话过期时，仍需重新登录。课表和待办页面都可以按住 Ctrl 滚动鼠标滚轮，分别调整大小。</p>
      <button onClick={()=>perform(api?.login)}>打开教务登录窗口</button><button className="quit" onClick={()=>perform(api?.quit)}>退出程序</button>
    </section>
  </div>}
 </main>;
}
