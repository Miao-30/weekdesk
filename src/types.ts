...
export type UITheme='soft'|'bright';
...
export interface Preferences { topmost:boolean;opacity:number;fontSize:number;todoFontSize:number;autoStart:boolean;showInactive:boolean;showWeekend:boolean;rememberLogin:boolean;theme:UITheme;todoSort:TodoSort;todoFilter:TodoFilter;bounds?:{x:number;y:number;width:number;height:number} }
